"""
=============================================================
🚀 SMART DOCUMENT Q&A — Premium RAG Application
=============================================================
Claude-like Premium Interface with Universal API Support

Supports: NVIDIA AI | Groq | OpenRouter (500+ Models)

Features:
- Module 1: OCR + Smart Chunking (500w, 20% overlap)
- Module 2: NVIDIA Embeddings + FAISS Vector Store
- Module 3: Semantic Cache (92% threshold, 0ms latency)
- Module 4: FAISS Retrieval + NVIDIA Re-ranking (8→3)
- Module 5: Universal LLM Generation (any provider)

Built with Streamlit | Universal AI | FAISS | EasyOCR
...
import re
import html
import requests  # 🛡️ FIX: Proper import instead of __import__('requests')
...
# ============================================================
# 🛡️ FIX F-16: Structured JSON logging setup (do this FIRST)
# ============================================================
...
try:
    if hasattr(st, 'secrets') and st.secrets:
        for key in ["NVIDIA_API_KEY", "GROQ_API_KEY", "OPENROUTER_API_KEY", "OWNER_NAME", "RAG_ALLOWED_EMAILS"]:
...
if _STREAMLIT_SECRETS.exists() and not _secrets_loaded:
    # Streamlit auto-loads this, but for non-Streamlit contexts (FastAPI shim):
    try:
        import tomllib
...
# ============================================================
# 🛡️ FIX F-10: Server-side rate limiting (Redis-backed)
# Previous: per-session state, bypassable by clearing cookies.
# Now: uses modules/rate_limiter.py which falls back to in-memory
# if Redis not available, but is shared across all Streamlit workers.
# ============================================================
...
def _check_rate_limit() -> bool:
    ...
    🛡️ F-10: Now uses server-side limiter (Redis or in-memory fallback).
    """
    user_id = get_user_id()
...
def _sanitize_error(error: Exception) -> str:
    """🛡️ FIX: Sanitize error messages to prevent API key leaks."""
    msg = str(error)
...
# Previous: only escaped &, <, > — quote chars leaked.
# Now uses html.escape() which handles all 5 dangerous chars.
# ============================================================
...
# 🛡️ FIX F-16: logging already set up at top of file via setup_logging().
# Removed basicConfig — JSON formatter is now active.
# logger variable also already defined at line 40.

# ============================================================
# ⚠️ CONFIGURE YOUR NAME HERE ⚠️
# ============================================================
OWNER_NAME = os.getenv("OWNER_NAME", "Built by Developer")  # ← APNA NAAM YAHAN DAALO
APP_TAGLINE = "AI-Powered Document Intelligence Platform"
...
if not is_logged_in() and not _show_admin_panel:
    ...
    with _login_col2:
        ...
        <div style="text-align: center; color: #64748B; font-size: 11px; margin-top: 16px;">
            ...
            &#128274; Passwords securely hashed hote hain (SHA256 + salt).
        </div>
        """, unsafe_allow_html=True)
    st.stop()
...
if CONFIG.PUBLIC_MODE:
    ...
    # ── Build social links separately (avoids f-string `and` issues) ──
    ...
    # Build the "Built by ..." line with optional links
    _social_html = f'Built by <b style="color:#F1F5F9;">{_builder_name}</b>'
    if _github_link:
        _social_html += f' &bull; <a href="{_github_link}" target="_blank" rel="noopener" style="color:#3B82F6; text-decoration: none;">GitHub</a>'
    if _linkedin_link:
        _social_html += f' &bull; <a href="{_linkedin_link}" target="_blank" rel="noopener" style="color:#3B82F6; text-decoration: none;">LinkedIn</a>'

    # ── Demo banner at top (clean, simple HTML) ──
    st.markdown(f"""
...
<style>
    ...
    * { font-family: 'Inter', -apple-system, sans-serif; }
    ...
    .sidebar-module .mod-header .mod-icon {
        font-size: 16px;
    }

    .sidebar-module .mod-desc {
        font-size: 11px;
        color: var(--text-muted);
        margin-top: 4px;
        padding-left: 24px;
    ...
    .tag-fast { background: rgba(245, 158, 11, 0.15); color: var(--accent-orange); }
    .tag-smart { background: rgba(139, 92, 246, 0.15); color: var(--accent-purple); }
    .tag-premium { background: rgba(239, 68, 68, 0.15); color: var(--accent-red); }
    .tag-popular { background: rgba(59, 130, 246, 0.15); color: var(--accent-blue); }
    ...
    @media (max-width: 768px) {
        ...
        .chat-assistant, .chat-cache-hit {
            margin-right: 0;
        }
        .chat-user .msg-body, .chat-assistant .msg-body, .chat-cache-hit .msg-body {
            font-size: 14px;
        }
        .stats-bar {
            gap: 8px;
    ...
    /* ============================================================
       STREAMLIT NATIVE ELEMENTS — Mobile Override
       ============================================================ */
    @media (max-width: 768px) {
        ...
        .stButton > button {
            width: 100%;
            min-height: 44px !important;  /* Touch-friendly 44px */
        }
        /* File uploader full width */
        .stFileUploader {
            width: 100%;
        }
        /* Text inputs bigger for touch */
        .stTextInput > div > div > input {
        ...
        /* Hide RAG pipeline on mobile (saves space) */
        .rag-pipeline-mobile-hide {
            display: none;
        }
        /* Quick question buttons wrap properly */
...
def init_session_state():
    ...
    if "initialized" not in st.session_state:
        ...
        # 🧠 Auto-detect best provider from available keys
        st.session_state.available_keys = detect_available_keys()
        best = get_best_available_provider()
        # Handle "auto" or invalid DEFAULT_PROVIDER values
...
# Previously: per-session instance via st.session_state — first user of every
# new session paid 30+ seconds for EasyOCR model load.
# Now: process-wide singleton, loaded once, shared across all sessions.
@st.cache_resource(show_spinner=False)
def _get_ocr_scanner_cached(_languages: tuple):
    """Singleton OCR scanner — loaded once per process, NOT per session."""
    from modules.ocr_scanner import OCRScanner
    return OCRScanner(languages=list(_languages))
...
def get_chunker():
    if "chunker" not in st.session_state or st.session_state.chunker is None:
        ...
        # 🧠 AUTO-CONFIG: Model ke hisab se chunk size set karo
...
def get_embedding_provider():
    if st.session_state.embedding_provider is None:
        ...
        # 🧠 AUTO-STRATEGY: Koi bhi key ho, embeddings kaam karegi!
...
def get_semantic_cache():
    if st.session_state.semantic_cache is None:
        ...
        # 🧠 AUTO-CONFIG: Model ke hisab se cache threshold
        llm = get_llm_provider()
...
def reset_provider():
    ...
    st.session_state.vector_store = None
    st.session_state.retrieval_engine = None
    st.session_state.chunker = None  # Chunker bhi reset (auto-config change hoga)
    st.session_state.semantic_cache = None  # Cache threshold bhi change hoga
...
def process_document(uploaded_file):
    """Upload → OCR → Chunk (auto-config) → Embed (auto-strategy) → FAISS Store

    🛡️ Hardened with:
    - F-07: File size enforcement (200MB limit)
    - F-05: Cache invalidation on document change
    - F-15: Empty-file detection
    - Robust error handling (specific exceptions, no bare except)
    """
    start_time = time.time()

    # 🛡️ F-07: File size enforcement
    # DEMO mode me bhi 200MB allow (local testing ke liye)
    try:
        file_size = uploaded_file.size
    except (AttributeError, TypeError):
        file_size = 0

    # Use the larger of MAX_FILE_SIZE_BYTES or DEMO_MAX_FILE_SIZE_MB
    ...
    # Warn for large files (but still process)
    ...
    try:
        ...
        if not extracted_text or len(extracted_text.strip()) < 10:
            ...
            st.info("💡 Tip: If this is a scanned PDF, ensure Tesseract OCR is installed.")
            return

        word_count = len(extracted_text.split())
        st.session_state.total_words = word_count

        # Step 1.2 & 1.3: Smart Chunking — 🧠 AUTO-CONFIG chunk size!
        with st.spinner(f"✂️ Smart chunking ({mc.chunk_size}w chunks, {mc.chunk_overlap}w overlap)..."):
            chunker = get_chunker()  # Auto-configured by model
        ...
        with st.spinner("📚 Storing in FAISS vector database..."):
            vector_store = get_vector_store()
            # ⚡ Only delete_all for the FIRST file (multi-file: append, don't overwrite!)
        ...
        # 🛡️ FIX F-05: Invalidate cache when document changes
        # Compute document hash for cache keying
        try:
        ...
        # 🛡️ FIX F-19: Persist FAISS index to disk (survives app restart)
        try:
            import os as _os
            faiss_dir = _os.path.expanduser("~/.cache/rag_app")
            vector_store.save_to_disk(
                _os.path.join(faiss_dir, "faiss.index"),
                _os.path.join(faiss_dir, "faiss_meta.json"),
            )
        ...
        # ⚡ Multi-file: append names, sum words
        if st.session_state.document_name:
            st.session_state.document_name += f", {uploaded_file.name}"
        else:
            st.session_state.document_name = uploaded_file.name

        # 📦 Compression info for user (if PDF was large)
        _compression_info = ""
        try:
...
def ask_question(query: str):
    ...
    try:
        # Get LLM provider first (needed for auto-config)
        llm = get_llm_provider()
        mc = llm.model_config
        ...
        # Step 4: Retrieval + Re-ranking — 🧠 AUTO-CONFIG top_k from model
        vector_store = get_vector_store()
        ...
        # Re-ranking (try NVIDIA re-ranker if available)
        ...
        # Step 5: Generation — with AUTO model switching on rate limit
        context_text = "\n\n".join([
...
{context_text}
        ...
        # 🔄 AUTO-SWITCH: Try current provider, fallback on rate limit
        ...
        # Check if rate limited → Auto-switch to another provider
...
def _try_fallback_providers(messages: list) -> Optional[Dict]:
    """
    🔄 AUTO-SWITCH: Jab current provider rate limit hit kare,
    toh automatically doosre available providers try karo.

    Priority: Groq → NVIDIA → OpenRouter (jiska key ho)
    """
    from modules.llm_provider import LLMProvider, detect_available_keys, PROVIDERS, MODEL_CONFIGS

    available = detect_available_keys()
    current = st.session_state.selected_provider

    # Build fallback list (skip current provider)
    ...
    for provider_name in fallback_order:
        try:
            ...
            # Get auto-config for the fallback model
            ...
            # Only accept non-error responses
...
def _rerank(query, raw_results, top_n=3):
    ...
    if top_n > 50:
        top_n = 50
    # 🛡️ FIX: Query length limit to prevent API abuse
    if len(query) > 2000:
        query = query[:2000]
    ...
    if nvidia_key and nvidia_key.startswith("nvapi-"):
        try:
            ...
            # 🛡️ FIX: Proper requests import (top-level) instead of __import__
            resp = requests.post(
            ...
            if resp.status_code == 200:
                ...
                # 🛡️ FIX: Log non-200 responses instead of silently swallowing
                logger.warning(f"Re-ranker returned status {resp.status_code}")
        except requests.Timeout:
            # 🛡️ FIX: Specific exception handling instead of bare except
            ...
            # 🛡️ FIX: Log the error instead of bare except: pass
...
<div class="top-nav">
    ...
    <div class="top-nav-right">
        ...
        <div class="owner-badge">{OWNER_NAME}</div>
    </div>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    # ============================================================
    # 🌐 PUBLIC MODE: Quota Display (top of sidebar)
    # ============================================================
    if CONFIG.PUBLIC_MODE and _quota_mgr is not None:
    ...
    ocr_lang_options = {
        ...
        'English + Hindi (⏳ Slow first time)': ['en', 'hi'],
    }
    selected_lang_label = st.selectbox(
        "OCR Language",
        options=list(ocr_lang_options.keys()),
        index=0,  # Default: English only!
        help="English = instant. Hindi = extra model download (100MB+). Select only if needed!",
        key="ocr_lang_select",
    )
    ...
    if st.session_state.get('ocr_languages') != new_langs:
        ...
        st.session_state.ocr_scanner = None  # Reset so new langs are used

    st.markdown("---")

    # ============================================================
    # 🔑 API KEYS — UI Input + .env (Both work!)
    # ============================================================
    st.markdown("#### 🔑 API Keys")

    # Check which keys already loaded from .env
    ...
    if _nvidia_loaded or _groq_loaded or _openrouter_loaded:
        st.markdown('<p style="color:#059669;font-size:11px;">✅ Keys detected from .env file! Enter new keys below to override.</p>', unsafe_allow_html=True)
    else:
        st.markdown('<p style="color:#DC2626;font-size:11px;">⚠️ No .env keys found. Enter any ONE key below to start!</p>', unsafe_allow_html=True)
    ...
    # 🛡️ SECURITY: API key inputs are type="password" (masked)
    # UI Input for API keys (overrides .env if entered)
    ...
    ui_openrouter_key = st.text_input("🌐 OpenRouter Key", type="password",
        value="", placeholder="✅ Already configured" if _openrouter_loaded else "sk-or-xxxxx (optional)",
        help="Get key: https://openrouter.ai/ → Keys")

    # 🛡️ SECURITY: Validate key format before setting
    ...
    # Set keys: UI input overrides .env only if valid
    ...
    if st.session_state.document_processed:
        ...
        <div style="background:#F0FDF4;border:1px solid #BBF7D0;border-radius:8px;padding:10px;margin-top:8px;">
            <div style="font-size:11px;color:#166534;"><b>📄 {st.session_state.document_name}</b></div>
...
# ============================================================
# SINGLE-PAGE LAYOUT — Upload + Chat together!
# ============================================================
# No more annoying tab switching!
...
# Upload Zone — REAL file uploader (no fake HTML dropzone!)
st.markdown("### 📄 Upload Document(s)")
st.markdown("""
<div style="color: #94A3B8; font-size: 13px; margin-bottom: 8px;">
    📂 <b>30+ Formats:</b> PDF, DOCX, XLSX, PPTX, CSV, TSV, JSON, XML, HTML, EPUB, RTF, ODT, TXT, MD, Images — Max 200MB — Multiple files supported!
</div>
""", unsafe_allow_html=True)

# Supported formats list from OCR scanner
...
# Manual validation (prevents StreamlitAPIException on rerun with cached files)
if uploaded_files:
    valid_files = []
...
if uploaded_files:
    ...
    for f in uploaded_files:
        ...
        <div style="background: #1A1F2E; border-radius: 10px; padding: 14px; border: 1px solid #1E293B;
            ...
            <div>
                <div style="font-size: 14px; font-weight: 600; color: #F1F5F9;">📎 {f.name}</div>
    ...
    if st.button("🚀 Process Document(s)", type="primary", use_container_width=True):
        ...
        if not has_any_key:
            st.error("❌ No API keys found! Add at least ONE key in .env file (NVIDIA_API_KEY, GROQ_API_KEY, or OPENROUTER_API_KEY)")
        else:
...
# ============================================================
# 💬 CHAT SECTION (same page, no tab switch!)
# ============================================================
st.markdown("---")
...
if not st.session_state.document_processed:
    ...
    # Chat Messages with EDIT functionality
    st.markdown('<div class="chat-container">', unsafe_allow_html=True)

    for msg_idx, msg in enumerate(st.session_state.chat_history):
        if msg["role"] == "user":
            # 🛡️ FIX F-02: Use html.escape() for full XSS protection
            # (previous code only escaped &, <, > — quotes leaked)
            ...
            # Build meta line — only show if there's actual data
            meta_parts = []
            ...
            # 🛡️ FIX: AI response ko HTML me inject mat karo (breaks on tables/code/HTML-like content)
            # Instead: render wrapper div with HTML, then use st.markdown() for the body
            # Streamlit's st.markdown() safely renders markdown (bold, tables, code) and escapes HTML
            raw_answer = msg["content"]

            # Render the chat bubble wrapper (header + meta + sources) with HTML
            st.markdown(f"""
            ...
            # 🛡️ Render AI response body using Streamlit's SAFE markdown renderer
            # This handles tables, code blocks, bold, lists — WITHOUT breaking HTML
            # And automatically escapes any HTML in the response (XSS protection)
            try:
    ...
    # ============================================================
    # ✏️ CHAT EDIT: Edit previous user messages
    # ============================================================
    ...
    # Query Input — Enter key submits!
    st.markdown("---")
    
    # ⚡ Enter key submit: Use text_input with on_change callback
    def _submit_query():
...
<div class="app-footer">
    <div class="owner-line">{OWNER_NAME}</div>
